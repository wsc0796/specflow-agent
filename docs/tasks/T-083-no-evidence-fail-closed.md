# T-083 — No-Evidence Fail-Closed Boundary

> Integration identity (2026-09-13): T-083, formerly PR #9 T-072. Source: `c6c893c3a723a3f62c33265394ed1fcbf1c6029b` at `docs/tasks/T-072-no-evidence-fail-closed.md`. Original branch names, commits, dates and standalone validation below remain historical evidence, not integrated acceptance.
...
- **REQ-083-1:** After evidence collection, a bundle with zero excerpts must
  stop the multi-agent run before any Agent executor is called.
- **REQ-083-2:** The CLI runner must return exit code `3` for this classified
  runtime failure.
- **REQ-083-3:** The output directory must contain a bounded failure manifest
...
- **REQ-083-4:** The early failure artifacts must use the existing atomic write,
  artifact-size, integrity-manifest, and `_COMPLETE` conventions.
- **REQ-083-5:** Runs with at least one valid evidence excerpt must preserve the
  existing multi-agent behavior.
- **REQ-083-6:** The Run API must persist the classified runner outcome as
...
- **AC-083-1:** Given a valid repository and a requirement with zero matching
  evidence excerpts, the multi-agent runner returns `3` and no Agent executor
  is invoked.
- **AC-083-2:** The generated failure manifest records
  `workflow_state="failed"`, `error="EVIDENCE_NOT_FOUND"`, and
  `stages_completed=0` without raw repository paths or exception details.
- **AC-083-3:** The early failure directory ends with valid
  `artifact-integrity.json` and `_COMPLETE` files.
- **AC-083-4:** The existing evidence-backed Mock run remains successful.
- **AC-083-5:** Focused tests, full pytest, Ruff lint, Ruff format, secret scan,
  and `git diff --check` pass.
- **AC-083-6:** A project-bound API Run with zero evidence returns its normal
