...
*最终执行版 · V5.3.2 Learning Effectiveness Patch*

*SpecFlow 单旗舰 · Evaluation First · Gap Driven · 30 天*

> 仓库采用记录（非源 Word 正文）：本 Markdown 于 2026-09-09 基于 V5.3.1 迭代，作为当前仓库学习路线真相源。当前采用快照仍为 `codex/t072-no-evidence-fail-closed@c0721ba`；若路线中的历史 commit 或实现假设与当前源码、测试、`CURRENT.md` 冲突，以当前仓库证据为准。
...
> V5.3.2 补丁范围：完整保留 V5.3.1 的 Evaluation First、Gap Driven、Dev/Holdout、Behavior Suite、Safety、ADR 与 Minimal Observability Slice。新增 Context 可见性与执行器访问边界、Evaluator Contract、Failure Attribution Ladder、可复现 Context 身份和 Retest Queue。补丁不增加天数，不新增开发主线，外部项目阅读只替换既有供体时间。
...
| 月底证据 | 代码 + Test + Trace + Benchmark + Holdout + Behavior Suite + ADR + 可复现 README + 2 至 3 条简历 Bullet |
...
| Evidence First | Day 8 至 14 先测现有系统，再决定第三周做什么 | 避免先写答案再找问题 |
...
| 外部供体 | 一个能力一个主参考；单次 1 至 2h；只回答一个问题 | 不追热点 |
| 证据前移 | Day 21 写技术事实，Day 28 Bullet，Day 29 至 30 攻击式答辩 | 月底不编故事 |
...
| OpenAI Agents SDK | Local Context / LLM Context / handoff filter / guardrail / trace | ≤1h 对照；不迁框架 |
| LangGraph | state schema / checkpointer / Store / subgraph / interrupt | 对照 ≤1.5h；durability micro-lab 仍 ≤2h（仅需要时） |
| Temporal Python Samples | Activity retry / replay / context propagation / side-effect durability | Day 24 只读 ≤1.5h；30 天不接入 |
| PydanticAI | typed tool / schema / untrusted history / span evaluator | ≤1h；只抽 validation/eval 思想 |
| Deep Agents | tool visibility 与 permission、subagent context、长线程压缩 | 可选 ≤45m；只读 Architecture；替换供体时间 |
| OpenTelemetry GenAI Conventions | Agent / Workflow / Tool Span 词汇与内容隐私 | Day 22 只读 ≤45m；Development 状态，不照搬字段 |
| MCP / A2A | 协议边界 | 知道定位；不做主线 |

> 外部参考时间采用替换制，不采用叠加制。一个能力只保留一个主参考和最多一个短对照；任何阅读若不能回答当前 SpecFlow 的一个明确问题，立即停止。
...
### 3.4 Evaluator Contract

每个进入正式报告的 Evaluator 必须记录：输入字段、输出口径、deterministic 或 LLM judge、一个应通过样本、一个应失败样本、已知盲区、人工复核方式和版本。没有经过正反例校准的 Rubric 只能标为 hypothesis，不能直接驱动架构修改。

每条 Agent Behavior Case 固定包含：

```text
must_happen
must_not_happen
legal_terminal_state
observable_evidence
```

行为断言优先使用确定性的 Test、Artifact 或 Span；LLM judge 只覆盖无法确定性裁决的语义部分。

## 4. 第一阶段：Understand & Correct（Day 1 至 7）
...
| 5 | 长上下文/冲突证据；区分 Runtime 可访问与 LLM 可见；追 Prompt 声明到实际消息；检查派生输出 provenance；供体对照采用替换制 | 四类 State 边界表 + Context Visibility Matrix + 1 个污染案例 |
| 6 | 追 Coordinator→Specialists→Synthesis/Review/Artifact；重点查 typed handoff / schema compatibility / integrity / receiver field minimization | Handoff 契约图 + 2 个失败假设 + 隔离边界判断 |
| 7 | 关闭资料 5 分钟讲解；重测 Day 4/5 partial 项；亲手复现一个 correctness failure 并提交最小 Patch；补 1 个 tool permission 反例 | Week 1 Evidence Pack + Retest Queue v1 + Control Matrix v1 + Patch/Test |
...
两个 Case 都必须附带同一条接收者隔离断言：合法 Handoff 只包含接收者声明字段；Schema 通过不能掩盖 raw Evidence、外层 Context 或无关前序输出的过度暴露。

### 4.2 Day 5 Context Visibility Matrix

| 数据 | Runtime 可访问 | LLM 可见 | Artifact 持久化 | 下游 Agent 可见 |
| --- | --- | --- | --- | --- |
| requirement | ? | ? | ? | ? |
...
填表必须以实际 `LLMRequest.messages`、Executor 收到的 Context、Artifact 写入路径和接收者 Schema 为证据。字段声明、`prompt_id`、注释或文件名不算执行事实。

## 5. 第二阶段：Measure（Day 8 至 14）
...
| Day | 任务 | 当天硬交付 |
| --- | --- | --- |
| 8 | 定义 Eval Contract 与 Evaluator Contract；创建 30 条 Repository Task 模板 + 10 条 Behavior Suite；为行为用例定义 must/must-not/terminal；冻结非目标、Dev/Holdout 协议 | Eval Contract v1 + Evaluator Contract v1 + Case IDs + 数据纪律 |
| 9 | 补 gold evidence / expected properties；为每个核心 Evaluator 补应通过与应失败样本；冻结 20 Dev + 10 Holdout；Holdout 密封 | Dataset v1 + Evaluator Fixtures + Holdout manifest |
...
| 12 | A/B 对比质量、Evidence、Risk、Token、Latency、LLM Calls；抽样人工复核 Evaluator 分歧；列差距而不是下结论 | A/B Trade-off Table + Evaluator Disagreement Log |
| 13 | 按 Failure Attribution Ladder 区分 Retrieval、Context Assembly、Agent Reasoning、Schema/Validation、Evaluator、Artifact/Persistence | Failure Attribution Report |
| 14 | 写 Evidence Retrieval ADR：A 当前 keyword 足够；B semantic gap 明确；C 数据不足。同步确定 Tool Risk L0 至 L3 policy v0；重测 Week 2 partial 项 | Retrieval ADR + Safety Policy v0 + Retest Queue v2 + Week 2 Gate |
...
### 5.1 Failure Attribution Ladder
...
每个失败先定位最早出现错误证据的层，再讨论方案。不得把 Context 截断、派生摘要污染、Evaluator 误判或 Artifact 写入错误统一归因为 Retrieval failure。

## 6. 第三阶段：Improve by Evidence（Day 15 至 21）
...
| 18 | 校准 strategy / no-answer threshold；只有排序问题才 Rerank | Prompt Injection / Context Pollution / Tool Misuse / Permission / DLP |
| 19 | Top-K / Chunk / Context Budget 对照；保留 complexity/cost | 成本/延迟优化；有数据再讨论并发/backpressure |
| 20 | 跑 10 条 Behavior Suite + 安全专用 Dev；验证 must/must-not/terminal；冻结 code/params/prompt/threshold/policy | 同左；冻结 code/params/prompt/policy |
| 21 | 完整 Dev → 冻结 Holdout 一次 → Behavior 回归 → 重测关键 partial 项 → 写 3 条技术事实 | 同左 |
...
| Prompt Injection / Context Pollution | 外部文本诱导修改 Tool 参数，或 raw Evidence 越过接收者边界进入下游 | 不提升权限；危险动作拒绝/审批；未声明 Context 不进入接收者 |
...
> 每条必须保留：must_happen → must_not_happen → actual trajectory → tool name/args → key observation → retry/approval/deny → legal terminal state。优先 deterministic grader，不把 LLM-as-Judge 扩成新项目。
...
## 8. 第四阶段：Production Semantics（Day 22 至 27）
...
| ID | 语义 | 约束 |
| --- | --- | --- |
| conversation_id | 一次持续多轮会话 | 可以包含多个 run；当前未实现时必须写 N/A，不得用 run_id 冒充 |
...
所有 Span 公共字段：run_id、step_id、operation、status、失败时的 error.type，以及 trace/span 基础上下文。start、end 和 duration 由 tracing/runtime 记录。model 与 token 只属于相关 GenAI Span，不向所有 Span 无差别复制。Context 与 provenance 身份按实际可用情况记录 `prompt_hash`、`context_hash`、`evidence_hash` 和 `input_schema_id/version`，不记录原始敏感 payload。
...
| --- | --- | --- |
| IDs、model/provider、latency、token count、tool_name、status、retry_count、error_type、prompt/version/hash、context/evidence hash、input schema version、KB/index version、app/commit version | API Key、Authorization Header、.env、Secret、完整敏感 Tool Result、未脱敏用户数据、默认完整 Prompt、默认完整模型 Response | redaction、sampling、access control、retention policy 必须明确 |
...
| Day | 任务 | 硬边界 / 交付 |
| --- | --- | --- |
| 22 | cms-flow 压缩到约 1h，只看 TopoScheduler / barrier 与 SideEffectRunner；用 OTel GenAI 词汇做 ≤45m 对照；约 1.5h 冻结 Minimal Observability Slice | observability-contract.md：ID、Span、Privacy、Context/Provenance Identity、Online Runtime Metrics、Architecture 图；cms-flow 两张机制卡；不搭完整 OTel 平台 |
...
| 24 | 保留版本、幂等、Citation 与 crash→checkpoint→resume；区分 per-run / per-thread / cross-thread persistence；新增 Resume Observability Validation，不要求恢复前后 trace_id 相同 | 用 run_id、checkpoint_id、resume_from 或 trace link 证明 A→checkpoint→resume→B；验证 side effect 不重复；无法关联时写 known limitation / ADR |
| 25 | 真实 Embedding + 真实 LLM + 代表性 Live E2E + Tool Policy/Approval Trace；新增 Runtime Metrics Record | runtime-record.json 或 runtime-report.md：total_latency、input/output_tokens、retry_count、model/provider、prompt/context/evidence/schema/KB/index version、terminal_state；当前无 streaming，TTFT=N/A |
...
## 9. 第五阶段：Portfolio Truth（Day 28 至 30）

| Day | 任务 | 交付 |
| --- | --- | --- |
| 28 | 把 Day 21 技术事实压成 2 至 3 条 Bullet；所有数字逐一回指报告/Trace/Test | Bullet v2 + Evidence Map |
| 29 | Retest Queue v4 + 攻击式面试：数据泄露？Context 隔离证明到哪层？Evaluator 是否可信？A/B 是否公平？threshold 如何来？为什么做/没做 Vector？Tool permission 为什么这样分层？Handoff integrity 怎么证？ | 答辩记录 + 最后必要修正 |
...
## 10. 每天 6 至 8 小时执行节奏与砍项顺序

| 时间 | 动作 | 要求 |
| --- | --- | --- |
| 30m | 主动回忆 | 不看笔记画链路 / 回答 3 个问题 |
| 45 至 60m | 必要概念 | 只补当天代码要用的知识 |
...
Retest Queue 只保存可能改变能力判断的 partial 或 unverified 项：
...
下一次重测日期
闭卷重测题
结果：verified / practicing / unverified
```

- Day 7：Evidence、Context、State、Isolation。
- Day 14：Eval Contract、Holdout、Failure Attribution。
- Day 21：A/B、Behavior、技术取舍。
- Day 29：无笔记攻击式答辩。

AI 整理的图、表和参考答案不能直接把状态提升为 verified；只有学习者自己的追链、预测、实验、关键 Diff、结果解释或闭卷答辩可以更新状态。

## 11. 最终验收清单

- [ ] SpecFlow branch / commit / baseline 记录明确；README 可复现。
- [ ] 能不看文档画出 Evidence → Context → Multi-Agent/Handoff → Artifact 主链。
- [ ] Context Visibility Matrix 能区分 Runtime 可访问、LLM 可见、Artifact 持久化和下游接收者可见；至少一条负向证据证明未声明 Context 不进入目标边界。
- [ ] Control Coverage Matrix 已完成，所有 × / unknown 有源码证据与处置结论。
- [ ] 核心 Evaluator 已有正反例、版本、已知盲区与人工分歧记录；没有把未校准 Rubric 当成架构结论。
...
- [ ] Tool Runtime Safety 有 L0 至 L3 / allow-deny-approval；敏感/不可逆动作没有仅依赖模型审批。
...
- [ ] Trace 或 Runtime Record 能关联 prompt/context/evidence/schema 身份；当前不存在的 conversation_id 明确标 N/A。
...
- [ ] Retest Queue 中所有 role-critical 项已达到 verified，或以 unverified 和下一证据需求诚实收束。
