...
(`docs/tasks/T-062-runtime-boundary-remediation.md`). 35 files, +1834/-100.
...
| `src/specflow/api_security.py` | RunRateLimiter counts only permit-holders (A2); shared quota defaults; allowlist/reviewer/API-key boundary; non-ASCII key fails closed (R) | A2, A1, R | Keep |
...
| `tests/test_api_security.py` | Allowlist recheck (A1); quota semantics (A2); Unicode key, oversized key, and runtime-default documentation guard (R) | A1, A2, R | Keep |
...
| `scripts/smoke_installed_wheel.py` | Repeatable installed-wheel smoke: clean venv, boot, mock run, parsed and integrity-checked artifact (R) | R | Keep |
| `.github/workflows/ci.yml` | Builds the wheel and runs the installed-wheel smoke on every PR (R) | R | Keep |
...
| `python scripts/smoke_installed_wheel.py` | PASS (parsed and integrity-checked artifact) |
| `git diff --check` | pass |
| GitHub Actions `smoke` job | configured; runs `uv build` then the installed-wheel smoke |
