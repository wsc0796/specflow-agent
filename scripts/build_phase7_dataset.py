...
TASKS: list[
    ...
    (
        "f01",
        "Orders: cancel endpoint validation",
        "为 app/orders.py 设计取消订单接口的输入校验与错误契约",
        "bug_fix",
        "easy",
        ("app/orders.py",),
    ...
    (
        "f02",
        "Orders: state transition guard",
        "为 app/orders.py 的订单状态流转设计不可达状态保护与失败语义",
        "bug_fix",
        "easy",
        ("app/orders.py",),
        ("orders", "state_machine"),
        ("非法状态流转",),
        ("状态机", "失败语义"),
        ("always works",),
    ),
    (
        "f03",
        "Orders: idempotent cancel",
        "设计订单取消的幂等性,重复取消返回一致结果",
        "new_feature",
        "easy",
        ("app/orders.py",),
        ("orders", "idempotency"),
    ...
    (
        ...
        "为 app/catalog.py 设计商品列表接口的分页与过滤契约",
        "new_feature",
        "easy",
        ("app/catalog.py",),
        ("catalog", "api"),
    ...
    (
        "f05",
        "Cache: TTL policy",
        "为 app/cache.py 设计缓存 TTL 与失效策略的规格",
        "new_feature",
        "easy",
        ("app/cache.py",),
        ("cache", "ttl"),
    ...
    (
        "f06",
        "Auth: login failure semantics",
        "为 app/auth.py 设计登录失败的统一错误语义",
        "bug_fix",
        "easy",
        ("app/auth.py",),
        ("auth", "api"),
    ...
    (
        "f07",
        "Main: route registration spec",
        "为 app/main.py 设计新增路由的注册与异常处理契约",
        "refactor",
        "easy",
        ("app/main.py",),
        ("api", "routing"),
    ...
    (
        "f08",
        "Tests: cancel coverage",
        "为订单取消设计 tests/test_orders.py 的测试矩阵",
        "test_infra",
        "easy",
    ...
    (
        "f09",
        "Cache + Catalog integration",
        "设计缓存层与商品目录的集成方案:命中/失效/回源",
        "new_feature",
        "medium",
        ("app/cache.py", "app/catalog.py"),
        ("cache", "catalog", "integration"),
        ("缓存击穿", "失效延迟"),
        ("集成契约", "失效语义"),
        ("always fresh",),
    ...
    (
        "f10",
        "Auth + Orders authorization",
        "设计订单取消的鉴权要求:未认证与越权场景",
        "security",
        "medium",
        ("app/auth.py", "app/orders.py"),
        ("auth", "orders", "security"),
        ("越权", "未认证"),
        ("鉴权规则", "越权测试"),
        ("production-ready",),
    ),
    (
        "f11",
        "Orders + Tests contract",
        "设计订单模块与测试层的契约:测试如何锁定行为",
        "test_infra",
        "medium",
    ...
    (
        "f13",
        "Catalog + Cache read path",
        "设计商品读路径:缓存优先、回源、降级",
        "new_feature",
        "medium",
        ("app/catalog.py", "app/cache.py"),
        ("catalog", "cache", "resilience"),
        ("降级失败", "数据不一致"),
        ("读路径契约", "降级策略"),
        ("never stale",),
    ),
    ...
    (
        "f15",
        "Orders + Catalog shared types",
        "设计订单与目录共享的类型与错误码模块",
        "refactor",
        "medium",
        ("app/orders.py", "app/catalog.py"),
        ("shared", "types"),
        ("类型漂移",),
        ("共享契约",),
        ("always works",),
    ...
    (
        "f16",
        "Cache + Auth token caching",
        "设计认证令牌的缓存策略(不缓存敏感信息)",
        "security",
        "medium",
        ("app/cache.py", "app/auth.py"),
        ("cache", "auth", "security"),
        ("敏感缓存",),
        ("缓存边界",),
        ("never stale",),
    ...
    (
        "f17",
        "Tests: cross-module flow",
        "设计 缓存→目录→订单 跨模块流程的集成测试计划",
        "test_infra",
        "medium",
        ("tests/test_orders.py", "app/cache.py", "app/catalog.py"),
        ("testing", "integration"),
        ("测试顺序依赖",),
        ("集成测试矩阵",),
        ("100% coverage",),
    ...
    (
        "f19",
        "Pagination config",
        "为列表接口设计分页配置(默认值/上限)与 pyproject 无关的运行时配置",
        "new_feature",
        "medium",
        ("app/catalog.py", "app/main.py"),
        ("api", "config"),
        ("配置缺失", "越界"),
        ("配置契约", "默认值"),
        ("production-ready",),
    ...
    (
        "f24",
        "Order cancel + API test plan",
        "为取消接口设计 API 级测试计划(成功/失败/边界)",
        "test_infra",
        "medium",
        ("app/orders.py", "tests/test_orders.py", "app/main.py"),
        ("testing", "api", "orders"),
        ("边界遗漏",),
        ("API 测试矩阵",),
        ("100% coverage",),
    ...
    (
        "f26",
        "Orders: bulk cancel contract",
        "设计批量取消接口的输入约束、部分失败与幂等",
        "new_feature",
        "hard",
        ("app/orders.py", "app/main.py"),
        ("orders", "api"),
        ("部分失败", "幂等"),
        ("批量契约", "部分失败语义"),
        ("production-ready",),
    ...
    (
        "f27",
        "Auth brute-force limiting",
        "设计登录限流与失败计数策略",
        "security",
        "hard",
        ("app/auth.py",),
        ("auth", "security", "rate_limit"),
        ("暴力破解", "限流误伤"),
        ("限流规则",),
        ("production-ready",),
    ),
    ...
    (
        ...
        "security",
        "hard",
        ("app/main.py", "app/orders.py", "app/auth.py"),
        ("security", "api"),
        ("信息泄露",),
        ("脱敏规则",),
        ("production-ready",),
    ...
    (
        ...
        "设计缓存/目录依赖不可用时的降级与失败边界",
        "reliability",
        "hard",
        ("app/cache.py", "app/catalog.py", "app/main.py"),
