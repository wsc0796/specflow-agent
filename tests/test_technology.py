...
def test_oversized_python_file_is_not_read(tmp_path: Path) -> None:
    ...
    assert result.application_entry_candidates == [p for p in result.application_entry_candidates
                                                     if "huge" not in p]
...
# ── SQLite comment edge case (not entry-candidate noise) ─────────
# "FastAPI(" inside a comment would still match with the current substring
# approach.  Proper comment-awareness requires a Python AST pass, which is
# overkill for low-stakes entry-candidate detection (a false positive here
# merely adds one harmless candidate line).  The v0.3.1 principle applies:
# prefer ruff/AST for important checks, accept low-cost false positives for
